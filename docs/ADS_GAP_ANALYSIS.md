...
| ADS-7.7-01 | 未引入 OpenTelemetry / OpenInference SDK，但**已留出端口** | 本地优先、单进程的桌面应用，引入完整追踪栈（及其依赖树）的收益低于成本。现在的形态是：`internal/ports/trace.go` 定义 `Tracer`/`Span` 端口，`internal/util/trace` 提供进程内记录器，`/api/status` 暴露最近/最慢请求与分段打点，HTTP 层带 `X-Request-ID` 关联 ID。**换成 OTLP 导出器只需换一个实现**，调用点与状态端点的断言（`ports.TraceSnapshotter`）都不用改。 |
...
1. **样式层**：`styles.css` 已拆成 18 个功能域文件（入口只剩 `@import` 链），并加了四项 CSS 门禁；
   剩余债是 245 处 `!important`（集中在两个断点段），门禁只保证不再增长。
2. **真实模型质量仍未闭环**：口径、归因、预算闸与手动评测工作流都已就位，
   但**结论本身仍需**在能连到网关的机器上跑 `release_eval.py` 产出 100 例证据。
3. **OpenTelemetry / span 树**：已留端口（见第 3 节），OTLP 导出是后续可选项。
